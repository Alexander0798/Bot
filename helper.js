...
module.exports = {
  ...
  info(msg) {
    if (msg.message) {
      return { userName: msg.from.first_name, userId: msg.from.id, chatId: msg.message.chat.id, text: msg.message.text, messageId: msg.message.message_id };
    }
  ...
  getDateReserved(reservedDate, month) {
    return {
      day: dayjs(reservedDate * 1000).tz('gmt').format('DD'),
        month: month[+dayjs(reservedDate * 1000).tz('gmt').format('MM') - 1],
        monthId: dayjs(reservedDate * 1000).tz('gmt').format('MM'),
        hour: dayjs(reservedDate * 1000).tz('gmt').format('HH'),
        minutes: dayjs(reservedDate * 1000).tz('gmt').format('mm'),
    }
  },
